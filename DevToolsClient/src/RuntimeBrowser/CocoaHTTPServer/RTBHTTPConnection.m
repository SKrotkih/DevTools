...
@interface RTBHTTPConnection (PrivateAPI)
...
@implementation RTBHTTPConnection
...
- (id)initWithAsyncSocket:(GCDAsyncSocket *)newSocket configuration:(HTTPConfig *)aConfig
{
	if ((self = [super init]))
	{
		...
		{
			connectionQueue = dispatch_queue_create("RTBHTTPConnection", NULL);
		}
...
- (NSString *)generateNonce
{
	...
	[NSTimer scheduledTimerWithTimeInterval:TIMEOUT_NONCE
	                                 target:[RTBHTTPConnection class]
...
/**
 * This method is called by the RTBHTTPServer if it is asked to stop.
 * The server, in turn, invokes stop on each RTBHTTPConnection instance.
**/
- (void)stop
{
	dispatch_async(connectionQueue, ^{
 ...
 * The RTBHTTPServer comes with two such classes: RTBHTTPFileResponse and RTBHTTPDataResponse.
 * RTBHTTPFileResponse is a wrapper for an NSFileHandle object, and is the preferred way to send a file response.
 * RTBHTTPDataResponse is a wrapper for an NSData object, and may be used to send a custom response.
**/
...
//	{
//		return [[[RTBHTTPFileResponse alloc] initWithFilePath:filePath forConnection:self] autorelease];
//	
...
//	return nil;
	...
	return [webServer responseForPath: path];
}
...
- (void)die
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:RTBHTTPConnectionDidDieNotification object:self];
}
...
- (id)initWithServer:(RTBHTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot
{
	if ((self = [super init]))
	{
...
- (id)initWithServer:(RTBHTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot queue:(dispatch_queue_t)q
{
	if ((self = [super init]))
	{
		server = [aServer retain];
