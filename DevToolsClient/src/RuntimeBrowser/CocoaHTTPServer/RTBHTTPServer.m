...
@interface RTBHTTPServer (PrivateAPI)
	...
	if ((self = [super init]))
	{
		...
		[[NSNotificationCenter defaultCenter] addObserver:self
		                                         selector:@selector(connectionDidDie:)
		                                             name:RTBHTTPConnectionDidDieNotification
		                                           object:nil];
 ...
 * The default connection class is RTBHTTPConnection.
 * If you use a different connection class, it is assumed that the class extends RTBHTTPConnection
**/
...
- (BOOL)stop
{
	...
	dispatch_sync(serverQueue, ^{
		...
		for (RTBHTTPConnection *connection in connections)
		{
			[connection stop];
		}
...
**/
- (NSUInteger)numberOfRTBHTTPConnections
{
	NSUInteger result = 0;
...
{
	RTBHTTPConnection *newConnection = (RTBHTTPConnection *)[[connectionClass alloc] initWithAsyncSocket:newSocket
...
/**
 * This method is automatically called when a notification of type RTBHTTPConnectionDidDieNotification is posted.
