...
/**
 * Should only return YES after the RTBHTTPConnection has read all available data.
 * That is, all data for the response has been returned to the RTBHTTPConnection via the readDataOfLength method.
**/
- (BOOL)isDone;
 ...
 * 
 * This method ties into the asynchronous response architecture of the RTBHTTPConnection.
 * You should read the full discussion at the bottom of this header.
 * 
 * If you return YES from this method,
 * the RTBHTTPConnection will wait for you to invoke the responseHasAvailableData method.
 * After you do, the RTBHTTPConnection will again invoke this method to see if the response is ready to send the headers.
 * 
...
/**
 * This method is called from the RTBHTTPConnection class when the connection is closed,
 ...
 * invoke any methods on the RTBHTTPConnection after this method is called (as the connection may be deallocated).
**/
 ...
 * 
 * RTBHTTPConnection supports asynchronous responses.  All you have to do in your custom response class is
 * asynchronously generate the response, and invoke RTBHTTPConnection's responseHasAvailableData method.
 ...
 * 
 * The normal flow of events for an RTBHTTPConnection while responding to a request is like this:
 ...
 * After returning YES from this method, the RTBHTTPConnection will wait until the response invokes its
 * responseHasAvailableData method. After this occurs, the RTBHTTPConnection will again query the delayResponseHeaders
 ...
 * Asynchronously generating the body of the response is not an excuse to delay sending the headers.
 * 
 * After the response headers have been sent, the RTBHTTPConnection calls your readDataOfLength method.
 * You may or may not have any available data at this point. If you don't, then simply return nil.
 * You should later invoke RTBHTTPConnection's responseHasAvailableData when you have data to send.
 * 
 ...
 * RTBHTTPConnection will automatically detect when it should be requesting new data and will act appropriately.
 * 
 ...
 * You should also be aware that the RTBHTTPConnection automatically sorts any range requests.
 * So if your setOffset method is called with a value of 100, then you can safely release bytes 0-99.
 * 
 * RTBHTTPConnection can also help you keep your memory footprint small.
 * Imagine you're dynamically generating a 10 MB response.  You probably don't want to load all this data into
 * RAM, and sit around waiting for RTBHTTPConnection to slowly send it out over the network.  All you need to do
 * is pay attention to when RTBHTTPConnection requests more data via readDataOfLength.  This is because RTBHTTPConnection
