...
NSString*  argTypeSpecifierForEncoding(char fc) 
{
	...
	if(fc == 'V') 
        return @"oneway ";
	if(fc == '!') 
        return @""; // garbage-collector marked invisible -> ignore
	return nil;
}
...
- (NSString* )parseStructOrUnionEndCh: (char)endCh 
                                depth: (int *)depth 
                                sPart: (int)sPart 
                               inLine: (BOOL)inLine 
                              inParam: (BOOL)inParam 
                           spaceAfter: (BOOL)spaceAfter;
- (NSDictionary *)typeEncParseObjectRefInStruct: (BOOL)inStruct 
                                     spaceAfter: (BOOL)spaceAfter;
- (NSDictionary *)cTypeDeclForEncTypeDepth: (int *)depth 
                                     sPart: (int)sPart 
...
+ (ClassDisplay *) createForClass: (Class) klass 
{
...
- (NSDictionary *)typeEncWarning: (NSString*) inParse 
                     startingIVT: (const char*) startingIVT 
                      origResult: (NSDictionary*) origResult 
{
...
- (NSString *)typeForFilerCode: (char)fc 
                    spaceAfter: (BOOL)spaceAfter 
{
    NSString* rs;
...
- (NSDictionary *)typeEncParseBitField: (BOOL)spaceAfter 
{
    ...
    if (sscanf(ivT, "%d", &sizeModifier) == 1) 
    {  // parse the integer
    ...
    {  // warn about badly formatted FILER type info.
        typeS = (spaceAfter ? @"unsigned int " : @"unsigned int");
        modifierS = @"/* : ? */";
        result = [NSDictionary dictionaryWithObjectsAndKeys:typeS, TYPE_LABEL, modifierS, MODIFIER_LABEL, nil];
        if (!currentWarning)
            result = [self typeEncWarning: @"bit field" 
                              startingIVT: (ivT - 1) 
                               origResult: result];
...
- (NSDictionary *)typeEncParseArrayOf: (int *)depth 
                                sPart: (int)sPart 
                               inLine: (BOOL)inLine 
                              inParam: (BOOL)inParam 
                           spaceAfter: (BOOL)spaceAfter 
{
    ...
    if (sscanf(ivT, "%d", &sizeModifier) == 1) 
    {  // Array encoding starts with the size of the array
        NSDictionary* innerTypeInfo;
		
        while (isdigit(*ivT)) ++ivT;      // move past the digits (size)
        innerTypeInfo = [self cTypeDeclForEncTypeDepth: depth 
                                                 sPart: sPart 
                                              inStruct: NO 
                                                inLine: inLine 
                                               inParam: inParam 
                                            spaceAfter: spaceAfter]; // what TYPE of array
    ...
    {  // no size encoded ... handle bad or unrecognized encodings
        if (!currentWarning) 
        {
            currentWarning = YES;
...
- (NSDictionary *)typeEncParsePointerTo: (int *)depth 
                                  sPart: (int)sPart 
                                 inLine: (BOOL)inLine 
                                inParam: (BOOL)inParam 
                             spaceAfter: (BOOL)spaceAfter 
{
    ...
    {
        NSDictionary* innerTypeInfo = [self cTypeDeclForEncTypeDepth: depth 
                                                               sPart: sPart 
                                                            inStruct: NO 
                                                              inLine: inLine 
                                                             inParam: inParam 
                                                          spaceAfter: spaceAfter]; // Get the type
...
- (NSDictionary *)flatCTypeDeclForEncType 
{
    ...
    return [self cTypeDeclForEncTypeDepth: &structDepth 
                                    sPart: structPart 
                                 inStruct: NO 
                                   inLine: YES 
                                  inParam: YES 
...
- (NSMutableString *)_parseUnnamedStructOrUnionVarEndCh: (char)endCh 
                                                  depth: (int *)depth 
                                                  sPart: (int)sPart 
                                                 inLine: (BOOL)inLine 
                                                inParam: (BOOL)inParam 
{
    ...
    for (i=1; *ivT != endCh; ++i) 
    {
		...
		structInfo = [self cTypeDeclForEncTypeDepth: depth 
                                              sPart: i 
                                           inStruct: YES 
                                             inLine: inLine 
                                            inParam: inParam 
        ...
        if (sPart > 1 || *depth > 1) 
        {   // PENDING -- make var (and arg, and category and ...) names a parameter
        ...
        NSString* str = [structInfo objectForKey: TYPE_LABEL];
        if (str)
        {
            [structS appendString: str];
        }
        if (partName)
        {
            [structS appendString: partName];
        }
        str = [structInfo objectForKey: MODIFIER_LABEL];
        if (str)
        {
            [structS appendString: str];
        }
...
- (NSDictionary *)typeEncParseStructOrUnionWithEncType: (NSString *)encType 
                                                 endCh: (char)endCh 
                                                 depth: (int *)depth 
                                                 sPart: (int)sPart 
                                                inLine: (BOOL)inLine 
		...
		(innerUPos == NULL || eqPos < innerUPos) && (innerSPos == NULL || eqPos < innerSPos) ) 
    {
        ...
        typeS = [self parseStructOrUnionEndCh: endCh 
                                        depth: depth 
                                        sPart: sPart 
                                       inLine: inLine 
                                      inParam: inParam 
    ...
    {   // named struct or union (name only)
        const char* tmp = strchr(ivT, endCh);
        if (tmp != NULL) 
        {
            ...
            if (*ivT != '?') 
            {
                ...
                if (endCh == ')') 
                {  // Learned this later... no longer a generic Struct/Unin parser. Alas.
                ...
                {
					NSString* s = [NSString stringWithCString: ivT 
                           ...
                           spaceAfter: (BOOL)spaceAfter 
{
    ...
    {  // usually means inParam == YES
        structS = [self _parseUnnamedStructOrUnionVarEndCh: endCh 
                                                     depth: depth 
                                                     sPart: sPart 
                                                    inLine: inLine 
                                                   inParam: inParam];
    ...
    { // PENDING -- do typedefs and refer to structs by name
        tmpS = [NSString stringWithFormat: @" { %@%@}", structS, depthS];
        [namedStructs setObject: tmpS 
                         forKey: name];
    }
                                ...
                                spaceAfter: (BOOL)spaceAfter 
{
    ...
    switch (*ivT) 
    {  // what sort of thing are we parsing
        case '!' :   // "weak" pointer specifier (for new Garbage collection...).
            ...
            result = [self cTypeDeclForEncTypeDepth: depth 
                                              sPart: sPart 
                                           inStruct: inStruct 
                                             inLine: inLine 
                                            inParam: inParam 
        ...
        case '^' :   // Pointer to another type.
            ++ivT;
            result = [self typeEncParsePointerTo: depth 
                                           sPart: sPart 
                                          inLine: inLine 
                                         inParam: inParam 
                                      spaceAfter: spaceAfter];
        ...
        case '@' :   // id or named object reference
            ++ivT;
            result = [self typeEncParseObjectRefInStruct: inStruct 
                                              spaceAfter: spaceAfter];
            break;
        case '[' :   // array
            ++ivT;
            result = [self typeEncParseArrayOf: depth 
                                         sPart: sPart 
                                        inLine: inLine 
                                       inParam: inParam 
                                    spaceAfter: spaceAfter];
        ...
        default :    // a simple type or starts with a type specifier
            while (isTypeSpecifier(*ivT)) 
            {  // prepend type specifier(s)
                if (typeSpec == nil)
                {
                    typeSpec = @"";
                }
                
                NSString* nextTypeSpec = argTypeSpecifierForEncoding(*ivT);

                if (nextTypeSpec != nil)
                {
                    typeSpec = [typeSpec stringByAppendingString: nextTypeSpec];
                    // typeSpec = [typeSpec stringByAppendingString:(inParam ? nextTypeSpec : nil)];
                }
                
                ++ivT;
            }
            if (typeSpec == nil) 
            { // most common case: types are NOT modified by a specifier
                type = [self typeForFilerCode: *ivT 
                                   spaceAfter: spaceAfter];
                modifier = @"";
                ++ivT;
            } 
            else 
            {
                ...
                NSString* nextTypeSpec = [result objectForKey: TYPE_LABEL];
                
                if (nextTypeSpec != nil)
                {
                    type = [typeSpec stringByAppendingString: nextTypeSpec];
                }
                else
                {
                    type = typeSpec;
    ...
    {
        if (*ivT == closingChar) 
        {  // Properly closed array, struct or union
            ++ivT;
        } 
        else 
        {            // handle bad or unrecognized encodings
            if (!currentWarning) // if we haven't already, include a warning...
                result = [self typeEncWarning: parsedTypeName 
                                  startingIVT: startingIVT 
                                   origResult: result];
...
- (NSDictionary *)typeEncParseObjectRefInStruct: (BOOL)inStruct spaceAfter: (BOOL)spaceAfter 
{
    ...
    if ((*ivT == '"') && (!inStruct || [self hasClassName])) 
    {  // '@' followed by '"' implies the class name is supplied.
        ++ivT;                      // skip the quote
        tmp = strchr(ivT, '"');     // go to the end of the quoted class name
        if (tmp != NULL) 
        {   // (should never happen) no end quote -- default to type 'id' and hope this is parsed elsewhere
...
{
	NSString *name = [NSString stringWithCString: property_getName(p) 
                                        encoding: NSUTF8StringEncoding];
	NSString* attr = [NSString stringWithCString: property_getAttributes(p) 
                                        encoding: NSUTF8StringEncoding];
	...
	for(NSString* comp in comps) 
    {
		...
		switch (c) 
        {
			...
			case 't':
			{
                ...
                if (d)
                {
                    NSString* str = [d valueForKey: TYPE_LABEL];
                    
                    if (str)
                    {
                        type = [NSString stringWithFormat: @"%@ ", str];
                    }
                }
	...
	if(getter) 
        [at addObject: [NSString stringWithFormat: @"getter=%@", getter]];
	if(setter) 
        [at addObject: [NSString stringWithFormat: @"setter=%@", setter]];
	if(memory) 
        [at addObject: memory];
	if(rw)     
        [at addObject: rw];
...
- (NSSet* )ivarsTypeTokens 
{
    ...
    NSMutableSet* ivarsTypeSet = [NSMutableSet set];
    ...
    if (ivarList != NULL && (ivarListCount>0)) 
    {
        ...
        for ( i = 0; i < ivarListCount; ++i ) 
        {
            ...
            if (rtIvar && ivar_getTypeEncoding(rtIvar)) 
            {
                ...
                //NSLog(@"---- %@", components);
				[ivarsTypeSet addObjectsFromArray: components];
            }
        }
    }
	free(ivarList);
...
- (NSString*) header: (NSMutableString*) aDependecies
{
    NSMutableString* header = [NSMutableString string];
    
    [header appendString: [NSString stringWithFormat: @"/* Generated by RuntimeBrowser.\n   Image: %s\n */\n\n%@%@%@",
    ...
    if (ivarList != NULL && (ivarListCount>0)) 
    {
        for ( i = 0; i < ivarListCount; ++i ) 
        {
            ...
            if (rtIvar && ivar_getTypeEncoding(rtIvar)) 
            {
                ...
                [aDependecies appendFormat: @"%@,", [cTypeDeclInfo objectForKey: TYPE_LABEL]];
    ...
    for(p = 0; p < propertyListCount; p++) 
    {
        ...
        NSString* propDescr = [self propertyDescription:prop];

        if (propDescr != nil)
        {
            [header appendString: propDescr];
        }
        else
        {
            NSString *name = [NSString stringWithCString: property_getName(prop)
                                                encoding: NSUTF8StringEncoding];

            NSLog(@"*** %@: property decription is nil!", name);
        }
	}
	free(propertyList);
    ...
    if (i>0) 
    {   // The last one in the list contains the original methods implemented by the class.
        NSString*  str = [classMethods objectAtIndex: (i-1)];
        
        if (str != nil)
        {
            [header appendString: str];
            [header appendString: @"\n"];
        }
        else
        {
            NSLog(@"*** classMethod is nil!");
        }
    }
    ...
    if (i>0) 
    {
        NSString* str = [instanceMethods objectAtIndex: (i-1)];
