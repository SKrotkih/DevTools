...
- (ClassStub *) getOrCreateClassStubsRecursivelyForClass: (Class) klass 
{
	...
	[allClassStubsByName setObject: cs 
                            forKey: klassName]; // Add it to our uniquing dictionary.
	...
	if (path) 
    {
        ...
        {
			[allClassStubsByImagePath setValue: [NSMutableArray array] 
                                        forKey: path];
	...
	if (parent != nil) 
    {               // and recursively create (or get) its stub.
...
{
	if ([allClassStubsByName count] == 0) 
    {
        [self rootClasses];        
    }
...
{
    //Classes are wrapped by ClassStub.  
    //This array contains wrappers for root classes (classes that have no superclass).
...
- (void) addToArray: (NSMutableArray*) anAllClasses forChildrenClasses: (NSArray*) aChildrenClasses 
{

    for (ClassStub* cs in aChildrenClasses) 
    {
        NSMutableDictionary* classItem = [NSMutableDictionary dictionary];
        [anAllClasses addObject: classItem];
        NSString* className = [cs description];        

        //NSLog(@"%@\n", className);
        
        NSMutableArray* children = [[cs children] mutableCopy];

        if ([children count] > 0) 
        {
            NSMutableArray* inheritClasses = [NSMutableArray array];

            [classItem setObject: inheritClasses 
                          forKey: className];

            [self addToArray: inheritClasses 
          forChildrenClasses: children];
        }
        else 
        {
            [classItem setObject: className 
                          forKey: className];
        }
...
- (NSString*) createDescription
{
    ...
    NSMutableArray* rootClassesArray = [NSMutableArray array];
    
    NSMutableDictionary* allClassesDict = [[NSMutableDictionary alloc] initWithObjectsAndKeys: rootClassesArray, @"Root", nil];

    NSArray* currRootClasses = [self rootClasses];
    
    NSData* jsonData = nil;
    
    if ([currRootClasses count] > 0) 
    {
        [self addToArray: rootClassesArray 
      forChildrenClasses: currRootClasses];
        
        jsonData = [NSJSONSerialization dataWithJSONObject: allClassesDict
                                                   options: NSJSONWritingPrettyPrinted // Pass 0 if you don't care about the readability of the generated string
                                                     error: &error];
    }
    
    [allClassesDict release];
