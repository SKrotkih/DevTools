...
-(id)initWithView: (UIView *)_targetView
{
	if ((self = [super initWithStyle: UITableViewStyleGrouped]))
    {
        ...
        {
			self.targetSubviews = [targetView performSelector: @selector(windows)];
		}
        else
        {
		...
		self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle: @"Exit"
                                                                                  ...
                                                                                  target: self
                                                                                  action: @selector(exit)] autorelease];
...
- (void) loadProperties
{
    NSMutableDictionary* props = [[UIQuery describe: targetView] mutableCopy];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        CALayer* const layer = subView.layer;
		...
		if (textSize.width + imageSize.width > 260)
        {
			...
			CGRect labelFrame = CGRectMake(0, imageSize.height, textSize.width, textSize.height);
            UILabel *textLabel = [[[UILabel alloc] initWithFrame: labelFrame] autorelease];
			textLabel.text = text;
...
- (void) tableView: (UITableView *) tableView didSelectRowAtIndexPath: (NSIndexPath *) indexPath
{
	if (indexPath.section == 0)
    {
        UIView* targView = [properties objectForKey: [[properties allKeys] objectAtIndex: indexPath.row]];
		[self.navigationController pushViewController: [[[UIInspector alloc] initWithView: targView] autorelease]
                                             animated: YES];
	}
    else
    {
        UIView* targView = [targetSubviews objectAtIndex: indexPath.row];
		[self.navigationController pushViewController: [[[UIInspector alloc] initWithView: targView] autorelease]
                                             animated: YES];
	}
}
