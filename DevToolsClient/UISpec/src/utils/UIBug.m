...
- (id)initWithPoint:(CGPoint)point
{
	if (!swizzleWasDone)
    {
		[UIBug swizzleMethodOnClass: [UIApplication class]
                   originalSelector: @selector(sendEvent:)
                          fromClass: [UIBug class]
                  alternateSelector: @selector(mySendEvent:)];
		swizzleWasDone = YES;
	}
	if (self = [super initWithImage: [UIImage imageNamed: @"UISpec.bundle/images/uibug.png"]
                   highlightedImage: [UIImage imageNamed: @"UISpec.bundle/images/uibug2.png"]])
    {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	UIActionSheet* actionSheet = [[[UIActionSheet alloc] initWithTitle: nil
                                                ...
                                                destructiveButtonTitle: nil
                                                     otherButtonTitles: @"Open Console", @"Enable Inspector", nil] autorelease];
