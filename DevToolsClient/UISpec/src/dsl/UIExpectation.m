...
+ (id) withValue: (const void*) aValue
        objCType: (const char*) aTypeDescription
            file: (const char*) aFile
            line: (int)aLine
   isFailureTest: (BOOL)failureTest
{
	if (*aTypeDescription == '@' && ([*(id*) aValue isKindOfClass:[UIQuery class]] || [*(id*) aValue isKindOfClass:[UIRedoer class]]))
    {
		return [[[UIQueryExpectation alloc] initWithValue: aValue
                                                 objCType: aTypeDescription
                                                     file: aFile
                                                     line: aLine
                                            isFailureTest: failureTest] autorelease];
	}
...
- (id) initWithValue: (const void*) aValue
            objCType: (const char*) aTypeDescription
                file: (const char*) aFile
                line: (int) aLine
       isFailureTest: (BOOL) failureTest
...
-(void)be:(SEL)sel {
	...
	BOOL result = [[value performSelector: sel] boolValue];
	...
	for (NSString *key in selectors) {
		if (![key isEqualToString:@""]) {
			...
			if ([returnType isEqualToString:@"@"]) {
				if ([(id)expected isKindOfClass:[NSString class]]) {
...
+(SEL)makeIsSelector:(SEL)aSelector
{
	...
	return NSSelectorFromString([NSString stringWithFormat: @"is%@", [selector stringByReplacingCharactersInRange: NSMakeRange(0,1)
                                                                                                       withString: [[selector substringWithRange:NSMakeRange(0,1)] uppercaseString]]]);
}
