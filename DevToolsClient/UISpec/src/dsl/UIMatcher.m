...
+ (id) withValue: (const void*) aValue
   ...
   matchSelector: (SEL) aMatchSelector
{
	return [[[self alloc] initWithValue: aValue
                               objCType: aTypeDescription
                          matchSelector: (SEL)aMatchSelector] autorelease];
}
...
{
	return (BOOL)[self performSelector: matchSelector
                            withObject: value];
}

- (BOOL) matches: (const void*) value
        objCType: (const char*) typeDescription
{
    return NO;
}
...
+ (NSString*) valueAsString: (const void*) value
                   objCType: (const char*) typeDescription
{
	if ('^' == *typeDescription)
    {
	...
	return [[NSNumber numberWithValue: value
                             objCType: typeDescription] stringValue];
}
