...
-(id)initWithViews:(NSMutableArray *)_views className:(NSString *)_className filter:(BOOL)_filter {
	if (self = [super init]) {
		self.timeout = 1;    // S.K. this value was 10 sec!
		self.views = _views;
...
-(id)templateFilter {
    ...
    NSString* viewClassName = [NSString stringWithFormat:@"UI%@", [viewName stringByReplacingCharactersInRange:NSMakeRange(0,1) withString:[[viewName substringWithRange:NSMakeRange(0,1)] uppercaseString]]];

	return [self view: viewClassName];
}
...
-(UIQuery *)view:(NSString *)aClassName {
	...
	while ([start timeIntervalSinceNow] > (0 - timeout)) {
		NSArray *actualViews = filter ? self.views : self.descendant.views;
		Class class = NSClassFromString(aClassName);
		for (UIView * v in actualViews)
        {
	...
	if ([aClassName isEqualToString:@"UITableViewCell"])
    {
		return [UIQueryTableViewCell withViews:array className:aClassName];
	}
    else if ([aClassName isEqualToString:@"UITableView"])
    {
		return [UIQueryTableView withViews:array className:aClassName];
	} 
	else if ([aClassName isEqualToString:@"UISearchBar"])
    {
		return [UIQuerySearchBar withViews:array className:aClassName];
	}
	else if ([aClassName isEqualToString:@"UITabBar"])
    {
		return [UIQueryTabBar withViews:array className:aClassName];
	}
	else if ([aClassName isEqualToString:@"UISegmentedControl"])
    {
		return [UIQuerySegmentedControl withViews:array className:aClassName];
	}
	else if ([aClassName isEqualToString:@"UIWebView"])
    {
		return [UIQueryWebView withViews:array className:aClassName];
	}
	else
    {
		return [UIQuery withViews:array className:aClassName];
	}
}
...
-(UIQuery *)flash {
    ...
    {
		UIColor *tempColor = [targetView.backgroundColor retain];
		...
		targetView.backgroundColor = tempColor;
		[tempColor release];
	}
...
-(UIQuery *)path
{
	...
	for (UIView *targetView in [self targetViews])
    {
		NSMutableString *targetPath = [NSMutableString stringWithString:@"\n"];
		NSArray *parentViews = [[UIParents withTraversal] collect:[NSArray arrayWithObject:targetView]];

		for (int i = parentViews.count-1; i>=0; i--)
        {
			[targetPath appendFormat:@"%@ -> ", [[parentViews objectAtIndex:i] class]];
		}
		[targetPath appendFormat:@"%@", [targetView class]];
...
+(void)show:(NSArray *)views
{
    ...
    {
		NSDictionary *dict = [self describe: view];
...
+ (NSDictionary*) describe: (id) object
{
	...
	do {
        ...
        NSLog(@"============ describing class %@", className);
		unsigned i;
		id objValue;
		int intValue;
		long longValue;
		...
		for (i=0; i<propertyCount; i++)
        {
            ...
            // Exceptional for work condition ():
			if ([key isEqualToString: @"domAdapter"] || [key isEqualToString: @"typingAttributes"])
            {
                continue;
            }
            
            NSString *keyAttributes = [NSString stringWithFormat:@"%s", propertyAttributes];
			NSLog(@"property[%i] = %@", i, key);
			NSLog(@"property attributes = %@", keyAttributes);
			...
			if ([object respondsToSelector:selector])
            {
				...
				NSString* invokeSelector = NSStringFromSelector([invocation selector]);
                NSLog(@"invocation selector = %@", invokeSelector);
				...
				@try
                {
                    ...
                    // Exceptional for work condition ():
                    if ([invokeSelector isEqualToString: @"currentSelectionRange"] || [invokeSelector isEqualToString: @"_caretRect"])
                    {
                        continue;
                    }
...
- (UIQuery *)touch {
	...
	for (UIView *targetView in [self targetViews])
    {
		...
		NSSet *touches = [[NSMutableSet alloc] initWithObjects: &targetTouch count: 1];
		
		[targetTouch.view touchesBegan: touches withEvent: eventDown];
		
		UIEvent *eventUp = [[NSClassFromString(@"UITouchesEvent") alloc] initWithTouch:targetTouch];
		[targetTouch setPhase: UITouchPhaseEnded];
		
		[targetTouch.view touchesEnded: touches
                             withEvent: eventDown];
...
- (id) initInView: (UIView*) view
{
    ...
    if (self != nil)
    {
        ...
        {
            frameInWindow = [view.window convertRect: view.frame
                                            fromView: view.superview];
        }
        
        _tapCount = 1;
        _locationInWindow = CGPointMake(frameInWindow.origin.x + 0.5 * frameInWindow.size.width,
...
- (void) propertiesOfObject: (id) object
{
	Class clazz = [object class];
