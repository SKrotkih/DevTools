...
-(void)onFinish:(int)count {
    ...
    if ([log length] > 0)
    {
        NSString* logErrorsTests = [NSString stringWithFormat: @"%@", log];
        [[NSNotificationCenter defaultCenter] postNotificationName: UITestErrorNotificationId
                                                            object: logErrorsTests];
    }
    
    NSString* logFinishTests = [NSString stringWithFormat: @"\n\nFinished in %f seconds", fabsf([start timeIntervalSinceNow])];

    [[NSNotificationCenter defaultCenter] postNotificationName: UITestLogNotificationId
                                                        object: logFinishTests];
    
	
    NSString* logSummaryTests = [NSString stringWithFormat: @"\n\n%i examples %d failures", count, errors.count];

    [[NSNotificationCenter defaultCenter] postNotificationName: UITestLogNotificationId
                                                        object: logSummaryTests];

    [[NSNotificationCenter defaultCenter] postNotificationName: UITestFinishedNotificationId
                                                        object: @""];
