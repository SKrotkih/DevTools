...
+(void)runSpecClasses:(NSArray *)specClasses {
	...
	int examplesCount = 0;
	[logger onStart];
	for (id aClass in specClasses)
    {
		Class class = (Class) aClass;
...
+ (void) runExamples: (NSArray*) examples
              onSpec: (Class) class
{
...
+ (NSDictionary*) specsAndExamples
{
	...
	for (Class specClass in specClasses)
    {
		...
		if ([examples count])
        {
			[specsAndExamples addObject: examples
                                 forKey: NSStringFromClass(specClass)];
		}
	}
...
+ (void) swizzleMethodOnClass: (Class) targetClass
...
+ (void) runScriptForCommandsArray: (NSArray*) anArray
{
    for (NSMutableString* currCommand in anArray)
    {
        $(currCommand);
    }
}
...
{
    [SpecsOutputData runScriptItem: aScriptItem];
}
